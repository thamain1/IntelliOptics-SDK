...
# ---------- Auth ----------
def require_bearer(token: Optional[str] = None):
...
async def image_query(
    ...
    start = time.time()
    # TODO: route to ML; for demo, return a deterministic answer
    answer: AnswerLabel = "UNCLEAR"
    conf = 0.50
    # simple heuristic: file name contains "yes" -> YES else NO
    try:
...
def image_query_json(payload: ImageQueryJson):
    # identical behavior to multipart endpoint; just returns a stub answer
    start = time.time()
