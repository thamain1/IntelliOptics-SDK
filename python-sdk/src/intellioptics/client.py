...
try:
    ...
    from .generated.openapi.api.default.get_detector_v1_detectors_detector_id_get import (
        sync as _get_detector_sync,
    )
    from .generated.openapi.api.default.get_image_query_v1_image_queries_iq_id_get import (
        sync as _get_image_query_sync,
    )
    from .generated.openapi.api.default.healthz_healthz_get import sync as _healthz_sync
    from .generated.openapi.api.default.image_query_json_v1_image_queries_json_post import (
        sync as _image_query_json_sync,
    )
    from .generated.openapi.api.default.image_query_v1_image_queries_post import sync as _image_query_sync
    ...
    from .generated.openapi.models.answer_out import AnswerOut
    ...
    from .generated.openapi.models.image_query_multipart import ImageQueryMultipart
    ...
    from .generated.openapi.types import File
except Exception:  # pragma: no cover - fallback when generated code missing
    AuthenticatedClient = _t.Any  # type: ignore[assignment]
    AnswerOut = _t.Any  # type: ignore[assignment]
    ...
    ImageQueryMultipart = _t.Any  # type: ignore[assignment]
    ...
    File = _t.Any  # type: ignore[assignment]
    ...
    _image_query_sync = None  # type: ignore[assignment]
...
class IntelliOptics:
    ...
    def __init__(
        ...
        try:
            self._client: AuthenticatedClient | None = AuthenticatedClient(
                base_url=base_url,
                token=token,
                verify_ssl=verify,
                timeout=timeout,
            )
        except Exception:  # pragma: no cover - generated client missing
            self._client = None
    ...
    # ---- detectors ----
    def create_detector(self, **kwargs: _t.Any) -> DetectorOut:
        if self._client is None or _create_detector_sync is None or DetectorCreate is _t.Any:
            raise RuntimeError("Generated client not available; re-run codegen or check installation.")
        body = DetectorCreate(**kwargs)
        result = _create_detector_sync(client=self._client, body=body)
        if not isinstance(result, DetectorOut):
            raise RuntimeError(f"Unexpected response from API: {result!r}")
        return result

    def get_detector(self, detector_id: str) -> DetectorOut:
        if self._client is None or _get_detector_sync is None or DetectorOut is _t.Any:
            raise RuntimeError("Generated client not available; re-run codegen or check installation.")
        result = _get_detector_sync(client=self._client, detector_id=detector_id)
        if not isinstance(result, DetectorOut):
            raise RuntimeError(f"Unexpected response from API: {result!r}")
        return result
    ...
    def submit_image_query(
        ...
        *,
        ...
        metadata: _t.Union[dict[str, _t.Any], str, None] = None,
        inspection_id: str | None = None,
    ) -> AnswerOut:
        if self._client is None or _image_query_sync is None or ImageQueryMultipart is _t.Any:
            raise RuntimeError("Generated client not available; re-run codegen or check installation.")

        detector_id = detector.id if hasattr(detector, "id") else str(detector)
        ...
        body = ImageQueryMultipart(
            ...
            image=File(payload=io.BytesIO(jpg), file_name="image.jpg", mime_type="image/jpeg"),
        )
        result = _image_query_sync(client=self._client, body=body)
        if not isinstance(result, AnswerOut):
            raise RuntimeError(f"Unexpected response from API: {result!r}")
        return result

    def submit_image_query_json(self, **kwargs: _t.Any) -> AnswerOut:
        if self._client is None or _image_query_json_sync is None or ImageQueryJson is _t.Any:
            raise RuntimeError("Generated client not available; re-run codegen or check installation.")
        body = ImageQueryJson(**kwargs)
        result = _image_query_json_sync(client=self._client, body=body)
        if not isinstance(result, AnswerOut):
            raise RuntimeError(f"Unexpected response from API: {result!r}")
        return result

    def get_image_query(self, iq_id: str) -> AnswerOut:
        if self._client is None or _get_image_query_sync is None or AnswerOut is _t.Any:
            raise RuntimeError("Generated client not available; re-run codegen or check installation.")
        result = _get_image_query_sync(client=self._client, iq_id=iq_id)
        if not isinstance(result, AnswerOut):
            raise RuntimeError(f"Unexpected response from API: {result!r}")
        return result

    # ---- feedback ----
    def submit_feedback(self, **kwargs: _t.Any) -> _t.Any:
    ...
    # ---- labels ----
    def add_label(self, **kwargs: _t.Any) -> LabelRecord:
        if self._client is None or _add_label_sync is None or LabelCreate is _t.Any:
            raise RuntimeError("Generated client not available; re-run codegen or check installation.")
        body = LabelCreate(**kwargs)
        result = _add_label_sync(client=self._client, body=body)
        if not isinstance(result, LabelRecord):
            raise RuntimeError(f"Unexpected response from API: {result!r}")
        return result
